...
### 新增
- **内容级去重（MD5）**：同名文件不再只比大小——通过 `get_media_info` 返回的下载地址取 `etag`（实测等于内容 MD5）与本地 MD5 比对，内容一致跳过、不同视为新版本加后缀另存上传；本地 MD5 未变的文件零接口调用直接跳过（受 `maxVerifyPerRun` 限流保护）
- **本地模式**：无凭证 / 知识库连接失败（含 `200002 skill auth failed`）时自动降级为「只扫描 + 生成目录」，配好凭证重跑即自动补传；支持 `--local-only` 强制本地模式
- **路径自适应**：扫描根 / 归档根 / 技能目录全部由 `os.homedir()` 推导，换机器、换用户名无需改代码；项目自带 `cos-upload.cjs` 作为官方上传脚本缺失时的回退
- **同名文件大小比对**（内容级去重的退化路径）：etag 取不到时按 `content-length` 比对——大小不同视为新版本，自动加 `_YYYYMMDDHHmmss` 时间戳后缀**另存上传**，知识库中的旧版本保留
